...
const ItemCard = ({ item }: { item: Item }) => {
	...
	return (
		<div className='item'>
			<div>
				{ingredients && (
					<button onClick={toggleShow} onFocus={(e) => e.target.blur()}>
						<img src={show ? "pointer-down.svg" : "pointer-up.svg"}></img>
					</button>
				)}
				<img
			...
			{!show && ingredients && (
				<div className='ingredients'>
					{ingredients.map((value, index) => {
						return <ItemCard item={value} key={index} />
					})}
				</div>
			)}
		</div>
...
const Calculator = ({ className }: { className?: string }) => {
	...
	useEffect(() => {
		...
		const fetchProficiencies = async () => {
			try {
				const response = await axios.get<Page<Proficiency>>(`${ENDPOINTS.PROFICIENCY}/all`, { signal: controller.signal })
			...
			try {
				if (proficiencies.length > 0) {
					for (const proficiency of proficiencies) {
						const response = await axios.get<Page<Item>>(`${ENDPOINTS.ITEM}/all`, { params: { proficiency: proficiency }, signal: controller.signal })
						if (response.data.content) {
			...
			{!result ? (
				<Form onSubmit={(e) => e.preventDefault()}>
					<div>
						...
						<Select name='proficiency' id='proficiency' onChange={(e) => setProficiency(e.currentTarget.value)} disabled={proficiencies.length === 0}>
							{!proficiency && (
					...
					<div>
						...
						<Input type='number' name='quantity' id='quantity' min={1} step={1} onChangeCapture={(e) => handleQuantity(e)} placeholder='Quantity' />
					</div>
					<div>
						<Button type='submit' onClick={handleCalculate} disabled={!proficiency || !itemName || !quantity} $primary>
							Calculate
						</Button>
					</div>
				</Form>
