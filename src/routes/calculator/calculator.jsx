...
function Calculator() {
  ...
  const handleClick = (e) => {
    const URL = 'http://localhost:3000'
    const name = itemSelect.current.value
    for (const i of items[proficiency]) {
      if (i.name !== name) continue
      console.log(`${URL}/items/${i.id}`)
      return fetch(`${URL}/items/${i.id}`)
        .then(rs => rs.json())
        .then(i => {
    ...
    <div className='container'>
      <div className='main-container'>
        ...
        <div className='form-container'>
          ...
          <select id='proficiencies' value={proficiency}
          onWheel={e => handleWheel(e)} onChange={e => setProficiency(e.target.value)}>
        ...
        <div className='form-container'>
          ...
          <select id='items' ref={itemSelect} value={item} disabled={proficiency === 'DEFAULT'}
          onWheel={e => handleWheel(e)} onChange={e => setItem(e.target.value)}>
            {
              ...
              {items[proficiency].map(i => (<option key={i.id} value={i.name}>{i.name}</option>))}
              </>
            }
          </select>
        </div>
        ...
        <div className='form-container'>
          ...
          <input type='number' ref={quantityInput} id='quantity' placeholder='Quantity here'
          value={quantity} min={1} max={999999}
          onInput={e => handleInput(e)} onWheel={e => handleWheel(e)}/>
        </div>
        
        <div>
          <button id='calculate' disabled={proficiency === item || !quantity ? true : undefined} onClick={e  => handleClick(e)}>Calculate</button>
        </div>
